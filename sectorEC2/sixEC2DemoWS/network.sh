...
## Bring up the network if it is not arleady up.
  ...
  # and join peers in org1 and org2 to the channel
 scripts/createChannel_step1.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
  if [ $? -ne 0 ]; then
    echo "Error !!! Create channel-step1 failed"
    exit 1
  fi
}

# step2 is used to update anchor peer in each organization
# default anchor peers include peer0.org1.example.com and peer0.org2.example.com
function createChannel_step2() {

  # update anchor peers in org1 and org2
 scripts/createChannel_step2.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
  if [ $? -ne 0 ]; then
    echo "Error !!! Create channel-step2 failed"
    exit 1
  fi
...
## this function is seperated into step1 and step2 in multi-host deployment
#############################################################################
function deployCC() {
...
function deployCC_step1() {

  scripts/deployCC_step1.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
## Call the script to isntall and instantiate a chaincode on the channel
function deployCC_step2() {

  scripts/deployCC_step2.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf channel-artifacts log.txt basic.tar.gz basic'

  fi
}

# clean dockers and docker network when './network.sh down' can not work normally on a unsuccessful deployment.
function networkClean() {
  echo "stop running dockers..."
  docker stop $(docker ps -aq)
  echo "remove obsolete exited dockers..."
  docker rm -f $(docker ps -aq)
  echo "clean docker containers..."
  docker network prune -f
  echo "clean obsolete and dangling docker volumes..."
  docker volume rm $(docker volume ls -qf dangling=true)
  echo "clean obsolete docker images"
  #docker rmi -f $(docker images -q)
  echo "delete system-genesis-block dir..."
...
# modified by Hao, Sep 6, 2020
# due to the unstalbe result of command "hostname -I"
HOST_STR=$(hostname -I)
# HOST_1="172.30.0.196"
# HOST_2="172.30.0.59"
# HOST_3="172.30.0.178"
...
if [ "$HOST_IP" == "$HOST_1" ]; then
  COMPOSE_FILE_BASE=docker/docker-compose-test-net-host-1.yaml
  COMPOSE_FILE_COUCH=docker/docker-compose-couch0-org1.yaml
elif [ "$HOST_IP" == "$HOST_2" ]; then
  COMPOSE_FILE_BASE=docker/docker-compose-test-net-host-2.yaml
  COMPOSE_FILE_COUCH=docker/docker-compose-couch1-org1.yaml
elif [ "$HOST_IP" == "$HOST_3" ]; then
    COMPOSE_FILE_BASE=docker/docker-compose-test-net-host-3.yaml
    COMPOSE_FILE_COUCH=docker/docker-compose-couch2-org1.yaml
elif [ "$HOST_IP" == "$HOST_4" ]; then
  COMPOSE_FILE_BASE=docker/docker-compose-test-net-host-4.yaml
  COMPOSE_FILE_COUCH=docker/docker-compose-couch0-org2.yaml
elif [ "$HOST_IP" == "$HOST_5" ]; then
  COMPOSE_FILE_BASE=docker/docker-compose-test-net-host-5.yaml
  COMPOSE_FILE_COUCH=docker/docker-compose-couch1-org2.yaml
elif [ "$HOST_IP" == "$HOST_6" ]; then
  COMPOSE_FILE_BASE=docker/docker-compose-test-net-host-6.yaml
  COMPOSE_FILE_COUCH=docker/docker-compose-couch2-org2.yaml
else
  echo "error: unrecognized hostIP"
...
echo "network.sh running mode:"$MODE
...
if [ "$MODE" == "up" ]; then
  ...
  echo "prepare cypto and gensis."
elif [ "$MODE" == "createChannel_step1" ]; then
  echo "Creating channel-step1 '${CHANNEL_NAME}'."
  echo
  ...
  echo
elif [ "$MODE" == "createChannel_step2" ]; then
  echo "Creating channel-step2 '${CHANNEL_NAME}'."
  echo
elif [ "$MODE" == "down" ]; then
  ...
  echo
elif [ "$MODE" == "deployCC_step1" ]; then
  echo "deploying chaincode on channel-step1 '${CHANNEL_NAME}'"
elif [ "$MODE" == "deployCC_step2" ]; then
  echo "deploying chaincode on channel-step2 '${CHANNEL_NAME}'"
else
  printHelp
  exit 1
fi
