...
if [ "$HOST_IP" == "$HOST_1" ]; then
	#export CORE_PEER_ADDRESS=129.63.205.110:7051
	#. scripts/envVar.sh
	#checkCommitReadiness 1 "\"Org1MSP\": true" "\"Org2MSP\": true"
	#checkCommitReadiness 2 "\"Org1MSP\": true" "\"Org2MSP\": true"
	commitChaincodeDefinition 1 2
#elif [ "$HOST_IP" == "144.92.239.71" -o "$HOST_IP" == "144.92.239.72" -o "$HOST_IP" == "144.92.239.73"  ]; then
	#export CORE_PEER_ADDRESS=144.92.239.71:7051
	#. scripts/envVar.sh
	#checkCommitReadiness 1 "\"Org1MSP\": true" "\"Org2MSP\": true"
	#checkCommitReadiness 2 "\"Org1MSP\": true" "\"Org2MSP\": true"
	#commitChaincodeDefinition 1 2
else
	echo "unknow org commit, not necessary!"
fi
...
if [ "$HOST_IP" == "$HOST_1" -o "$HOST_IP" == "$HOST_2" -o "$HOST_IP" == "$HOST_3" ]; then
	...
	echo "unknow org queryCommitted, not necessary!"
fi


## Invoke the chaincode - this does require that the chaincode have the
## 'initLedger' method defined
if [ "$CC_INIT_FCN" = "NA" ]; then
	echo "===================== Chaincode initialization is not required ===================== "
else
	echo "===================== Chaincode initialization is required ===================== "
	if [ "$HOST_IP" == "$HOST_1" ]; then
		echo "invoking chaincode in org1"
		chaincodeInvokeInit 1 2
	elif [  "$HOST_IP" == "$HOST_4" ]; then
		echo "invoking chaincode in org2"
		chaincodeInvokeInit 1 2
	else
		echo "invoking chaincode in org3"
...
if [ "$HOST_IP" == "$HOST_1" ]; then
	echo "invoking init chaincode in org1"
	chaincodeInvokeInit 1 2
fi

echo "===================== Chaincode query ===================== "
if [ "$HOST_IP" == "$HOST_2" -o "$HOST_IP" == "$HOST_3" ]; then
	echo "Querying chaincode on org1..."
	chaincodeQuery 1
elif [ "$HOST_IP" == "$HOST_4" -o "$HOST_IP" == "$HOST_5" -o "$HOST_IP" == "$HOST_6" ]; then
	echo "Querying chaincode on org2..."
	chaincodeQuery 2
else
	echo "Querying chaincode on org3..."
	#chaincodeQuery 3
fi
