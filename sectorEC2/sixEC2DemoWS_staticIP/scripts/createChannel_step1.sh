...
if [ "$HOST_IP" == "$HOST_1" ]; then
	...
	# this is to use configtx.yaml in dir:~/fabric-samples/sixHostDemo/configtx
	FABRIC_CFG_PATH=${PWD}/configtx
  # Create channeltx
	echo "### Generating channel configuration transaction '${CHANNEL_NAME}.tx' ###"
	createChannelTx
	# Create anchorpeertx
	echo "### Generating anchor peer transaction '${CHANNEL_NAME}.tx' ###"
	createAncorPeerTx
	...
	createChannel
	echo "================== create ${CHANNEL_NAME} successfully! ================== "
fi
...
if [ "$HOST_IP" == "$HOST_1" -o "$HOST_IP" == "$HOST_2" -o "$HOST_IP" == "$HOST_3" ]; then
	...
	joinChannel 1
	echo "================== ${PEER_NAME} join myChannel successfully! ================== "
	...
	echo "Do not join Org3 peers to the channel..."
	# joinChannel 3
